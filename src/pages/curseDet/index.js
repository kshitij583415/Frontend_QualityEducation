...
export default function EduviCoursesPage() {
    ...
    const { standard } = location.state;
    const [oneCourses, setOneCourses] = React.useState([]);
    useEffect(() => {
        const fetchCourses = async () => {
            try {
                const res = await fetch("http://localhost:9090/getAllCourses");
                const data = await res.json();
                setOneCourses(data.filter(course => course.standard === "one"));
            }
    ...
    return (
        <>
            ...
            <div className="flex flex-col items-center justify-start w-full gap-[100px] bg-gray-100">
                ...
                <div className="ml-40 mr-40 justify-center w-[85%] gap-10 grid-cols-2 md:grid-cols-1 md:gap-5 grid">
                    {oneCourses.map((course, index) => (
