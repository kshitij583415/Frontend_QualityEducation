...
export default function AllmentorsPage() {
  ...
  useEffect(() => {
    const fetchAllUsers = async () => {
      try {
        const response = await fetch("http://localhost:9090/getAllUsers");
        const data = await response.json();
        console.log(data);
        const mentorUsers = data.filter((user) => user.isMentor === true);
          ...
          <div className="flex flex-wrap gap-4 mx-auto">
            {mentors.map((mentor, index) => (
              <AllMentorsMaincard
                ...
                foundermentor="Mentor" // Assuming occupation is available in your user entity
                className="flex flex-col items-center justify-start w-[23%] md:w-full gap-2.5"
              />
            ))}
          </div>
